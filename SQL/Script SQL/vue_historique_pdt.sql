-- ==========================================================
-- VUE : view_historique_portefeuille_pdt
-- OBJECTIF : Reconstitution quotidienne de la valeur et de la performance par produits
-- ==========================================================

CREATE OR REPLACE VIEW view_historique_portefeuille_pdt AS
WITH date_range AS (
...
mouvements_cumules AS (
    ...
    SELECT 
		...
		COALESCE(SUM(CASE 
		    ...
		    WHEN gv.pdt_cash = False AND mvt.mvt_nb_parts > 0 AND mvt.mvt_type_mouvement NOT IN ('ABONDEMENT') 
